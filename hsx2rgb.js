...
function lchToLab(l, c, h) {
    l *= 100; // Scale from [0,1] to [0,100]
    c *= 128; // Scale from [0,1] to [0,128]
    h *= 360; // Scale from [0,1] to [0,360]
    
    const a = c * Math.cos(degreesToRadians(h));
    const b = c * Math.sin(degreesToRadians(h));
    return [l, a, b];
}

// Conversion: CIELAB to XYZ
function labToXYZ(l, a, b) {
    const D65 = [0.95047, 1.00000, 1.08883]; // D65 illuminant
    const y = (l + 16) / 116;
    const x = y + a / 500;
    const z = y - b / 200;
    return [
